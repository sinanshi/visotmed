...
# 		 ))
 ...
 plotNA<-function(vMap){
	 ...
	 matrix[is.na(vMap@MapMatrix)==FALSE]<-NA
...
visoPutPoints<-function(vPoint){}
 ...
 visoColScale<-function(vMap,vPO){
 	  ...
 	  mai=c(current_mai[1], par("fin")[1]-par("mai")[4]+0.1,current_mai[2]+0.2,current_mai[4]-0.2),
	...
	print("--------")
        plot.new()
         ...
         axis(4,at=seq(0,1,length=vPO@colorscale.tickle),labels=label,las=1,tck=-0.3,line=0,cex.axis=2)
 ...
 visoImageSingle<-function(vMap,vPP, vPO,vGP, vLine=NULL, vPoly=NULL, vPoint=NULL,... ){
	  ...
	  fin<-par("fin")
	  par(mai=c(0.15*fin[2],0.15*fin[1],0.15*fin[2],0.15*fin[1]))
	 ...
	 plot.new()
	 plotNA(vMap)
	 image(vMap@MapMatrix,x=vMap@Long, y=vMap@Lat, col=COLS(vPO@color.levels),
	axes=F,xlab="",ylab="", xlim=vPO@xlim,ylim=vPO@ylim)#xlim=xlim_,ylim=ylim_)
	 rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col="white")

	
	 #par(new=TRUE)
	 image(vMap@MapMatrix,x=vMap@Long, y=vMap@Lat, col=COLS(vPO@color.levels),
	axes=F,add=T,xlab=F,ylab=F, xlim=vPO@xlim,ylim=vPO@ylim)
         map(add=T)
        
#          axis(1,las=1,line=2,cex.axis=1.5)
#  	 axis(2,las=3,line=0.1,cex.axis=1.5)
#          axis(1)
         ...
         title(vPP@title)#,line=1,cex=vPO@cex.main,adj=0)
